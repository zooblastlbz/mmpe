...
#SBATCH --output=/mnt/petrelfs/libozhou/mmpe/output/pretrain_without_mmpe/finetune/%j.out
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
    ...
    --pretrain_mm_mlp_adapter /mnt/petrelfs/libozhou/mmpe/output/pretrain_without_mmpe/pretrain_llava_vicuna/mm_projector.bin \
    ...
    --bf16 True \
    --output_dir /mnt/petrelfs/libozhou/mmpe/output/pretrain_without_mmpe/finetune \
    --num_train_epochs 1 \
