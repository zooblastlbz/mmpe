...
#SBATCH --output=/mnt/petrelfs/libozhou/mmpe/output/direct_finetune_direct_match_/%j.out
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
    ...
    --bf16 True \
    --output_dir "/mnt/petrelfs/libozhou/mmpe/output/direct_finetune_direct_match_" \
    --num_train_epochs 1 \
