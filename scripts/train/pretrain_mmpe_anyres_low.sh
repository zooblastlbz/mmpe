...
#SBATCH --output=/mnt/petrelfs/libozhou/mmpe/output/low_mmpe/pretrain/%j.out
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
    ...
    --bf16 True \
    --output_dir /mnt/petrelfs/libozhou/mmpe/output/low_mmpe/pretrain \
    --num_train_epochs 1 \
    --only_448 False \
    ...
    --report_to wandb \
    --run_name pretrain_mmpe_anyres_low \
